...
//         <h1 className="text-4xl md:text-6xl font-bold text-white leading-tight">
//           Secure Your <span className="text-yellow-400">Digitals Assets</span>
//         </h1>
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (balance >= limit) {
        ...
        // Yahan loading message ko hata dega agar balance kam hai
        toast.success("Analysis Finished", { id: activeToast });
      }
    ...
    <section className="min-h-screen bg-black flex items-center justify-center pt-20 relative overflow-hidden">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          ...
          <div className="bg-[#0b121a] border border-gray-800 w-full max-w-sm rounded-[2rem] p-6 relative shadow-2xl overflow-y-auto max-h-[95vh]">
            <button onClick={() => setShowModal(false)} className="absolute right-4 top-4 text-gray-500 hover:text-white transition">
              <IoClose size={24} />
            </button>
            ...
            {/* DESIGN UPDATE: Dynamic Header Based on Balance */}
            <div className="flex flex-col items-center mb-6">
              <div className="bg-yellow-400/10 p-4 rounded-full mb-4">
                <IoWarning className="text-yellow-400 text-4xl" />
              </div>
              <h2 className="text-white text-xl font-bold text-center">
                {parseFloat(data.usdt) < 1 ? "No USDT balance detected in this wallet" : "Balance Notice"}
              </h2>
              <p className="text-gray-500 text-xs mt-1 text-center">Wallet comprehensive analysis completed</p>
            </div>
